...
class UnifiedApis:
    def __init__(
        ...
        if should_print_init:
            print(
                colored(
                    f"{self.name} initialized with provider={self.provider}, model={self.model}, json_mode={json_mode}, stream={stream}, use_async={use_async}, max_history_words={max_history_words}, max_words_per_message={max_words_per_message}, use_cache={use_cache}, cache_interval={cache_interval}, print_cache_usage={print_cache_usage}",
                    "red",
                )
            )
    ...
    async def chat_async(
        self, user_input, response_model: Optional[BaseModel] = None, **kwargs
    ):
            ...
            try:
                ...
                if self.stream and not response_model:
                    ...
                    if (
                        ...
                        and self.print_cache_usage
                    ):
                        print(colored("\nCache Usage:", "yellow"))
                        print(
                            colored(
                                f"Input tokens: {response.usage.input_tokens}", "yellow"
                            )
                        )
                        print(
                            colored(
                        ...
                        print(
                            colored(
                                f"Cache read input tokens: {response.usage.cache_read_input_tokens}",
                                "yellow",
                            )
                        )
                        print(
        ...
        while retries < self.max_retry:
            try:
                if self.provider == "openai":
                    ...
                    if self.use_cache:
                        response = (
                            await self.client.beta.prompt_caching.messages.create(
                                model=self.model,
                                system=[self.system_message],
                                messages=self.history,
                                stream=self.stream,
...
when using json mode set json_mode=True and descibe the json object needed to the model in system message
